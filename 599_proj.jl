...
# ╔═╡ c58624a8-f0d4-4b54-9916-45555ed5465d
md"### upload gas sensor data and data processing functions
"

# ╔═╡ d5c471c3-26be-46c0-a174-d580d0ed7f7d
md"!!! example \"\"
	Declare utility data structures to be used in data processing.
"
...
md"!!! example \"\"
	Function to identify henry's law coefficient for gasses at low pressures:
"
...
function isotherm_data(mof::String, gas::String; remove_old_pressure_cols::Bool=true)
	# read in data as a data frame
	filename = isotherm_filename(mof, gas)
	data = CSV.read(filename, DataFrame, comment="#")
	
	# unit conversions in pressure
	if "P(torr)" in names(data)
		data[:, "P(bar)"] = data[:, "P(torr)"] / 750.069
	end
	
	if "P(kPa)" in names(data)
		data[:, "P(bar)"] = data[:, "P(kPa)"] * 0.01
	end
	
	if "P(activity)" in names(data)
		@assert gas == "H2O"
		
		p_vapor = 0.0 # vapor pressure of water.
		# from: https://en.wikipedia.org/wiki/Vapour_pressure_of_water
		if mof == "ZIF-8" # 298 K
			p_vapor = 3.1690 * 0.01 # bar
		elseif mof == "ZIF-71" # 35 deg C
			p_vapor = 5.6267 * 0.01 # bar
		else
			error("need vapor pressure of water")
		end
		data[:, "P(bar)"] = data[:, "P(activity)"] * p_vapor
	end
	
	if ! ("N(g/g)" in names(data))
		data[:, "N(g/g)"] = data[:, "N(mmol/g)"] / 1000.0 * gas_to_molecular_wt[gas]
	end
	
	# assert we got the right col's!
	cols_we_want = ["P(bar)", "N(g/g)"]
	for col in cols_we_want
		@assert col in names(data)
	end
	
	if remove_old_pressure_cols
...
!!! example \"\"
			  (1) Why is CO2 a uniform distribution instead of normal?

!!! example \"\"
			  (2) how were the standard deviations, means for normal distributions and beginning/end for uniform chosen?
...
md"!!! example \"\" 
	I don't know if it matters, changed the order of the distributions in the matrix to line up with the gases vector."
...
# ╔═╡ 3d3c229f-3415-4dc5-a6e6-6511d0104228
md"!!! example \"\" 
	I noticed the X axis of the H2O distribution is labeled RH, is that relative humidity? Why visualize H2O differently than CO2 and Ethylene (ppm)"
...
md"!!! example \"\" 
	I wonder if a vector of matricies would be easier to work with?"
...
# ╔═╡ 5e5f3bf0-9ce3-4e4f-92cf-fff5c962f30c
#=
...
md"!!! example \"\" 
	Playing with one class support vector machine"
...
begin
	...
	ax1 = Axis(contour_fig[1,1], xlabel = "m, " * mofs[1] * "[g/g]",
			   ylabel = "m, " * mofs[2] * "[g/g]",
				aspect = DataAspect(),
				title = "one class SVM contour for anomaly detection")
	
	prediction_map = heatmap!(feature_space_grid_axes[:,1], 
      feature_space_grid_axes[:,2], grid_predictions, colormap = color_map, colorrange 
      = (minimum(grid_predictions), maximum(grid_predictions)) )

	scatter!(m[1, :], m[2, :], strokewidth=1, color=(:white, 0.0))
	scatter!(m_anomaly[1, :], m_anomaly[2, :])

	contour!(feature_space_grid_axes[:, 1], feature_space_grid_axes[:, 2], grid_predictions, levels=[0.0], color=:yellow)
